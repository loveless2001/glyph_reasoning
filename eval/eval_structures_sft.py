...
def evaluate_model(model_name, tasks):
    ...
    try:
        ...
        is_adapter = os.path.exists(os.path.join(model_name, "adapter_config.json"))
        
        if is_adapter:
            print(f"Detected adapter at {model_name}, loading base model first...")
            ...
            # Load tokenizer from adapter path (to get added tokens)
...
def main():
    ...
    parser.add_argument("--output", default="eval/eval_results_sft.csv", help="Output CSV file path")
    parser.add_argument("--data", default="eval/tasks.json", help="Input data file (json or jsonl)")
    ...
    for model_name in args.models:
        ...
        if args.clear_cache:
            # Only attempt to clear from cache if it looks like a repo ID (no slashes)
            if "/" in model_name and not os.path.exists(model_name): 
                 # This logic is weak for paths vs keys, but generally local paths have slashes.
                 # HuggingFace IDs also have slashes (User/Repo). 
                 # Better to just skip clearing for local dirs.
                 pass
